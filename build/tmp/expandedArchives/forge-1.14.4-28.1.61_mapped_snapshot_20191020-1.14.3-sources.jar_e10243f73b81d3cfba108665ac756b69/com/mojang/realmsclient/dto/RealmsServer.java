...
public class RealmsServer extends ValueObject {
   ...
   private static void sortInvited(RealmsServer p_sortInvited_0_) {
      Collections.sort(p_sortInvited_0_.players, new Comparator<PlayerInfo>() {
         public int compare(PlayerInfo p_compare_1_, PlayerInfo p_compare_2_) {
            return ComparisonChain.start().compare(p_compare_2_.getAccepted(), p_compare_1_.getAccepted()).compare(p_compare_1_.getName().toLowerCase(Locale.ROOT), p_compare_2_.getName().toLowerCase(Locale.ROOT)).result();
         }
      });
   }
