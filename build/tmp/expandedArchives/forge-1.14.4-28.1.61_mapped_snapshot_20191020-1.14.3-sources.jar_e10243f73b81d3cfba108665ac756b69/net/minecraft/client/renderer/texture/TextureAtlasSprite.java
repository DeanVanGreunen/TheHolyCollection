...
public class TextureAtlasSprite implements net.minecraftforge.client.extensions.IForgeTextureAtlasSprite {
   ...
   protected TextureAtlasSprite(ResourceLocation locationIn, PngSizeInfo sizeIn, @Nullable AnimationMetadataSection animationMetadataIn) {
      ...
      if (animationMetadataIn != null) {
         Pair<Integer, Integer> pair = func_217788_b(animationMetadataIn.getFrameWidth(), animationMetadataIn.getFrameHeight(), sizeIn.width, sizeIn.height);
         this.width = pair.getFirst();
         this.height = pair.getSecond();
         if (!func_217787_a(sizeIn.width, this.width) || !func_217787_a(sizeIn.height, this.height)) {
            throw new IllegalArgumentException(String.format("Image size %s,%s is not multiply of frame size %s,%s", this.width, this.height, sizeIn.width, sizeIn.height));
         }
      } else {
   ...
   private static Pair<Integer, Integer> func_217788_b(int p_217788_0_, int p_217788_1_, int p_217788_2_, int p_217788_3_) {
      if (p_217788_0_ != -1) {
         return p_217788_1_ != -1 ? Pair.of(p_217788_0_, p_217788_1_) : Pair.of(p_217788_0_, p_217788_3_);
      } else if (p_217788_1_ != -1) {
         return Pair.of(p_217788_2_, p_217788_1_);
      } else {
         int i = Math.min(p_217788_2_, p_217788_3_);
         return Pair.of(i, i);
      }
   }

   private static boolean func_217787_a(int p_217787_0_, int p_217787_1_) {
      return p_217787_0_ / p_217787_1_ * p_217787_1_ == p_217787_0_;
   }
   ...
   private static int blendColors(int col0, int col1, int col2, int col3, boolean transparent) {
      if (transparent) {
         ...
         float f3 = 0.0F;
         if (col0 >> 24 != 0) {
            f += getColorGamma(col0 >> 24);
            f1 += getColorGamma(col0 >> 16);
            f2 += getColorGamma(col0 >> 8);
            f3 += getColorGamma(col0 >> 0);
         }

         if (col1 >> 24 != 0) {
            f += getColorGamma(col1 >> 24);
            f1 += getColorGamma(col1 >> 16);
            f2 += getColorGamma(col1 >> 8);
            f3 += getColorGamma(col1 >> 0);
         }

         if (col2 >> 24 != 0) {
            f += getColorGamma(col2 >> 24);
            f1 += getColorGamma(col2 >> 16);
            f2 += getColorGamma(col2 >> 8);
            f3 += getColorGamma(col2 >> 0);
         }

         if (col3 >> 24 != 0) {
            f += getColorGamma(col3 >> 24);
            f1 += getColorGamma(col3 >> 16);
            f2 += getColorGamma(col3 >> 8);
            f3 += getColorGamma(col3 >> 0);
         }
   ...
   public void func_217789_a(int p_217789_1_, int p_217789_2_, int p_217789_3_, int p_217789_4_) {
      this.x = p_217789_3_;
      this.y = p_217789_4_;
      this.minU = (float)p_217789_3_ / (float)p_217789_1_;
      this.maxU = (float)(p_217789_3_ + this.width) / (float)p_217789_1_;
      this.minV = (float)p_217789_4_ / (float)p_217789_2_;
   ...
   public String toString() {
      ...
      return "TextureAtlasSprite{name='" + this.iconName + '\'' + ", frameCount=" + i + ", x=" + this.x + ", y=" + this.y + ", height=" + this.height + ", width=" + this.width + ", u0=" + this.minU + ", u1=" + this.maxU + ", v0=" + this.minV + ", v1=" + this.maxV + '}';
   }
