...
public class IngameGui extends AbstractGui {
   ...
   protected void renderScoreboard(ScoreObjective objective) {
      ...
      for(Score score1 : collection) {
         ...
         this.getFontRenderer().drawString(s2, (float)k2, (float)j1, 553648127);
         this.getFontRenderer().drawString(s3, (float)(k1 - this.getFontRenderer().getStringWidth(s3)), (float)j1, 553648127);
         if (k == collection.size()) {
            fill(k2 - 2, j1 - 9 - 1, k1, j1 - 1, i1);
            fill(k2 - 2, j1 - 1, k1, j1, l);
            this.getFontRenderer().drawString(s, (float)(k2 + j / 2 - i / 2), (float)(j1 - 9), 553648127);
         }
      }
