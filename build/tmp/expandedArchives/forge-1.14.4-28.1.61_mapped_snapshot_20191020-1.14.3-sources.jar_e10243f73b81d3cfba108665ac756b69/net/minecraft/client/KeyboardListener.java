...
public class KeyboardListener {
   ...
   private boolean processKeyF3(int key) {
      if (this.debugCrashKeyPressTime > 0L && this.debugCrashKeyPressTime < Util.milliTime() - 100L) {
         ...
         case 67:
            ...
            this.setClipboardString(String.format(Locale.ROOT, "/execute in %s run tp @s %.2f %.2f %.2f %.2f %.2f", DimensionType.getKey(this.mc.player.world.dimension.getType()), this.mc.player.posX, this.mc.player.posY, this.mc.player.posZ, this.mc.player.rotationYaw, this.mc.player.rotationPitch));
            return true;
         case 68:
         ...
         case ENTITY:
            ...
            if (privileged) {
               if (askServer) {
                  ...
                  this.setEntityClipboardString(resourcelocation, vec3d, compoundnbt);
                  this.printDebugMessage("debug.inspect.client.entity");
               }
            } else {
               this.setEntityClipboardString(resourcelocation, vec3d, (CompoundNBT)null);
               this.printDebugMessage("debug.inspect.client.entity");
            }
         }
   ...
   public void onKeyEvent(long windowPointer, int key, int scanCode, int action, int modifiers) {
      if (windowPointer == this.mc.mainWindow.getHandle()) {
         ...
         } else if (InputMappings.isKeyDown(Minecraft.getInstance().mainWindow.getHandle(), 67) && InputMappings.isKeyDown(Minecraft.getInstance().mainWindow.getHandle(), 292)) {
         ...
         if ((!(this.mc.currentScreen instanceof ControlsScreen) || ((ControlsScreen)inestedguieventhandler).time <= Util.milliTime() - 20L)) {
            ...
            if (this.mc.gameSettings.keyBindFullscreen.matchesKey(key, scanCode)) {
               ...
               this.mc.gameSettings.fullscreen = this.mc.mainWindow.isFullscreen();
               return;
            }
         ...
         if (this.mc.currentScreen == null || this.mc.currentScreen.passEvents) {
            ...
            if (action == 0) {
               ...
               if (this.mc.currentScreen == null) {
                  if (key == 256) {
                     boolean flag2 = InputMappings.isKeyDown(Minecraft.getInstance().mainWindow.getHandle(), 292);
                     this.mc.displayInGameMenu(flag2);
                  }

                  flag1 = InputMappings.isKeyDown(Minecraft.getInstance().mainWindow.getHandle(), 292) && this.processKeyF3(key);
   ...
   public void setupCallbacks(long window) {
      InputMappings.func_216505_a(window, this::onKeyEvent, this::onCharEvent);
   }

   public String getClipboardString() {
      return this.field_216821_c.func_216487_a(this.mc.mainWindow.getHandle(), (p_197966_1_, p_197966_2_) -> {
   ...
   public void setClipboardString(String string) {
      this.field_216821_c.func_216489_a(this.mc.mainWindow.getHandle(), string);
   }
