...
public class FilledProfileResult implements IProfileResult {
   ...
   public List<DataPoint> getDataPoints(String sectionPath) {
      ...
      for(String s : this.timesMap.keySet()) {
         if (s.length() > sectionPath.length() && s.startsWith(sectionPath) && s.indexOf(30, sectionPath.length() + 1) < 0) {
      ...
      for(String s1 : set) {
         if (s1.length() > sectionPath.length() && s1.startsWith(sectionPath) && s1.indexOf(30, sectionPath.length() + 1) < 0) {
