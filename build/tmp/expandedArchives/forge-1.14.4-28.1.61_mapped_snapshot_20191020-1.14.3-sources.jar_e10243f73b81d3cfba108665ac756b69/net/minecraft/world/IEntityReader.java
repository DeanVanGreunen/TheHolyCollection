...
public interface IEntityReader {
   ...
   default PlayerEntity getClosestPlayer(double x, double y, double z) {
      ...
      for(PlayerEntity playerentity1 : this.getPlayers()) {
         if (EntityPredicates.NOT_SPECTATING.test(playerentity1)) {
            double d1 = playerentity1.getDistanceSq(x, playerentity1.posY, y);
   ...
   default PlayerEntity getClosestPlayer(EntityPredicate predicate, LivingEntity target) {
      return this.getClosestEntity(this.getPlayers(), predicate, target, target.posX, target.posY, target.posZ);
   }
   ...
   @Nullable
   default PlayerEntity getClosestPlayer(EntityPredicate predicate, double p_217359_2_, double p_217359_4_, double p_217359_6_) {
      return this.getClosestEntity(this.getPlayers(), predicate, (LivingEntity)null, p_217359_2_, p_217359_4_, p_217359_6_);
   }
   ...
   default List<PlayerEntity> getTargettablePlayersWithinAABB(EntityPredicate predicate, LivingEntity target, AxisAlignedBB box) {
      ...
      for(PlayerEntity playerentity : this.getPlayers()) {
         if (box.contains(playerentity.posX, playerentity.posY, playerentity.posZ) && predicate.canTarget(target, playerentity)) {
