...
public class AnvilSaveConverter {
   ...
   static boolean func_215792_a(Path p_215792_0_, DataFixer p_215792_1_, String p_215792_2_, IProgressUpdate p_215792_3_) {
      ...
      WorldInfo worldinfo = SaveFormat.func_215779_a(p_215792_0_, p_215792_1_, p_215792_2_);
      ...
      func_215794_a(new File(file2, "region"), list1, biomeprovidertype.create(biomeprovidertype.createSettings().setBiome(Biomes.NETHER)), list.size(), i, p_215792_3_);
      func_215794_a(new File(file3, "region"), list2, biomeprovidertype.create(biomeprovidertype.createSettings().setBiome(Biomes.THE_END)), list.size() + list1.size(), i, p_215792_3_);
      ...
      SaveHandler savehandler = SaveFormat.func_215783_a(p_215792_0_, p_215792_1_, p_215792_2_, (MinecraftServer)null);
   ...
   private static void func_215793_a(File p_215793_0_, File p_215793_1_, BiomeProvider p_215793_2_, int p_215793_3_, int p_215793_4_, IProgressUpdate p_215793_5_) {
      ...
      try (
         RegionFile regionfile = new RegionFile(p_215793_1_);
         RegionFile regionfile1 = new RegionFile(new File(p_215793_0_, (new StringBuilder()).append(s.substring(0, s.length() - ".mcr".length())).append(".mca").toString()));
      ) {
         for(int i = 0; i < 32; ++i) {
            for(int j = 0; j < 32; ++j) {
