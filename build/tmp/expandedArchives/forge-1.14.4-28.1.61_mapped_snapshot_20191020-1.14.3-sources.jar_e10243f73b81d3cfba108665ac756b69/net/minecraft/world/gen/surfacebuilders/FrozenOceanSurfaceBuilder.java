...
public class FrozenOceanSurfaceBuilder extends SurfaceBuilder<SurfaceBuilderConfig> {
   ...
   public void buildSurface(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, SurfaceBuilderConfig config) {
      ...
      float f = biomeIn.func_225486_c(blockpos$mutableblockpos.setPos(x, 63, z));
      ...
      for(int j1 = Math.max(startHeight, (int)d0 + 1); j1 >= 0; --j1) {
         ...
         if (blockstate1.isAir()) {
            ...
            if (j1 < seaLevel && (blockstate == null || blockstate.isAir())) {
               if (biomeIn.func_225486_c(blockpos$mutableblockpos.setPos(x, j1, z)) < 0.15F) {
                  blockstate = ICE;
               } else {
            ...
            if (j1 >= seaLevel - 1) {
               ...
               chunkIn.setBlockState(blockpos$mutableblockpos, blockstate2, false);
            }
         } else if (j > 0) {
            --j;
            chunkIn.setBlockState(blockpos$mutableblockpos, blockstate2, false);
   ...
   public void setSeed(long seed) {
      if (this.seed != seed || this.field_205199_h == null || this.field_205200_i == null) {
         ...
         this.field_205199_h = new PerlinNoiseGenerator(random, 4);
         this.field_205200_i = new PerlinNoiseGenerator(random, 1);
      }
