         ...
         if (this.isLiquidInStructureBoundingBox(worldIn, structureBoundingBoxIn)) {
            ...
            for(int j1 = 0; j1 < this.sectionCount; ++j1) {
               ...
               if (randomIn.nextInt(100) == 0) {
                  this.generateChest(worldIn, structureBoundingBoxIn, randomIn, 2, 0, k1 - 1, LootTables.CHESTS_ABANDONED_MINESHAFT);
               }

               if (randomIn.nextInt(100) == 0) {
                  this.generateChest(worldIn, structureBoundingBoxIn, randomIn, 0, 0, k1 + 1, LootTables.CHESTS_ABANDONED_MINESHAFT);
               }
            ...
            for(int l2 = 0; l2 <= 2; ++l2) {
               for(int i3 = 0; i3 <= i1; ++i3) {
                  int k3 = -1;
                  BlockState blockstate3 = this.getBlockStateFromPos(worldIn, l2, -1, i3, structureBoundingBoxIn);
                  if (blockstate3.isAir() && this.getSkyBrightness(worldIn, l2, -1, i3, structureBoundingBoxIn)) {
                     int l3 = -1;
                     this.setBlockState(worldIn, blockstate, l2, -1, i3, structureBoundingBoxIn);
                  }
               }
            }
            ...
            if (this.hasRails) {
               ...
               for(int j3 = 0; j3 <= i1; ++j3) {
                  BlockState blockstate2 = this.getBlockStateFromPos(worldIn, 1, -1, j3, structureBoundingBoxIn);
                  if (!blockstate2.isAir() && blockstate2.isOpaqueCube(worldIn, new BlockPos(this.getXWithOffset(1, j3), this.getYWithOffset(-1), this.getZWithOffset(1, j3)))) {
                     float f = this.getSkyBrightness(worldIn, 1, 0, j3, structureBoundingBoxIn) ? 0.7F : 0.9F;
                     this.randomlyPlaceBlock(worldIn, structureBoundingBoxIn, randomIn, f, 1, 0, j3, blockstate1);
                  }
               }
            }
            ...
            for(int i = this.boundingBox.minX; i <= this.boundingBox.maxX; ++i) {
               for(int j = this.boundingBox.minZ; j <= this.boundingBox.maxZ; ++j) {
                  if (this.getBlockStateFromPos(worldIn, i, this.boundingBox.minY - 1, j, structureBoundingBoxIn).isAir() && this.getSkyBrightness(worldIn, i, this.boundingBox.minY - 1, j, structureBoundingBoxIn)) {
                     this.setBlockState(worldIn, blockstate, i, this.boundingBox.minY - 1, j, structureBoundingBoxIn);
                  }
               }
            }
