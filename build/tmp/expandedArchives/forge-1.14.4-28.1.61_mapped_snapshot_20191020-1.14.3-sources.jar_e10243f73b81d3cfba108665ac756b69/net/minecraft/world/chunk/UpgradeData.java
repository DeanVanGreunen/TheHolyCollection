      ...
      if (p_196991_0_.getUpgradeData().field_196995_b.remove(p_196991_1_)) {
         ...
         for(BlockPos blockpos : BlockPos.getAllInBoxMutable(k, 0, i1, l, world.getHeight() - 1, j1)) {
            ...
            for(Direction direction : adirection) {
               blockpos$mutableblockpos.setPos(blockpos).move(direction);
               blockstate1 = func_196987_a(blockstate1, direction, world, blockpos, blockpos$mutableblockpos);
            }
      ...
      try (
         ...
         for(int i = 0; i < 16; ++i) {
            ...
            if (chunksection != null && aint != null && aint.length > 0) {
               Direction[] adirection = Direction.values();
               BlockStateContainer<BlockState> blockstatecontainer = chunksection.getData();

               for(int j : aint) {
                  int k = j & 15;
                  int l = j >> 8 & 15;
                  int i1 = j >> 4 & 15;
                  blockpos$pooledmutableblockpos.setPos(chunkpos.getXStart() + k, (i << 4) + l, chunkpos.getZStart() + i1);
                  BlockState blockstate = blockstatecontainer.get(j);
                  BlockState blockstate1 = blockstate;

                  for(Direction direction : adirection) {
                     blockpos$pooledmutableblockpos1.setPos(blockpos$pooledmutableblockpos).move(direction);
                     if (blockpos$pooledmutableblockpos.getX() >> 4 == chunkpos.x && blockpos$pooledmutableblockpos.getZ() >> 4 == chunkpos.z) {
                        blockstate1 = func_196987_a(blockstate1, direction, iworld, blockpos$pooledmutableblockpos, blockpos$pooledmutableblockpos1);
                     }
                  }

                  Block.replaceBlock(blockstate, blockstate1, iworld, blockpos$pooledmutableblockpos, 18);
               }
            }
         }
                  ...
                  if (blockstate.get(BlockStateProperties.DISTANCE_1_7) >= j) {
                     ...
                     if (i != 7) {
                        for(Direction direction : field_208827_f) {
                           blockpos$mutableblockpos.setPos(blockpos).move(direction);
                           BlockState blockstate1 = p_208826_1_.getBlockState(blockpos$mutableblockpos);
                           if (blockstate1.has(BlockStateProperties.DISTANCE_1_7) && blockstate.get(BlockStateProperties.DISTANCE_1_7) > i) {
      ...
      STEM_BLOCK(Blocks.MELON_STEM, Blocks.PUMPKIN_STEM) {
         public BlockState func_196982_a(BlockState p_196982_1_, Direction p_196982_2_, BlockState p_196982_3_, IWorld p_196982_4_, BlockPos p_196982_5_, BlockPos p_196982_6_) {
            if (p_196982_1_.get(StemBlock.AGE) == 7) {
               ...
               if (p_196982_3_.getBlock() == stemgrownblock) {
