...
public abstract class LightEngine<M extends LightDataMap<M>, S extends SectionLightStorage<M>> extends LevelBasedGraph implements IWorldLightListener {
   ...
   protected BlockState getBlockAndOpacity(long worldPos, @Nullable AtomicInteger opacityOut) {
      if (worldPos == Long.MAX_VALUE) {
         if (opacityOut != null) {
            opacityOut.set(0);
         }
         ...
         int i = SectionPos.toChunk(BlockPos.unpackX(worldPos));
         int j = SectionPos.toChunk(BlockPos.unpackZ(worldPos));
