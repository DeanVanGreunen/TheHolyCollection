      ...
      return this.fixTypeEverywhereTyped("EntityEquipmentToArmorAndHandFix", this.getInputSchema().getType(TypeReferences.ENTITY), this.getOutputSchema().getType(TypeReferences.ENTITY), (p_207448_4_) -> {
         ...
         if (optional1.isPresent()) {
            ...
            if (!dynamic.get("ArmorDropChances").get().isPresent()) {
               Dynamic<?> dynamic2 = dynamic.emptyMap().merge(dynamic.createFloat(iterator.next().asFloat(0.0F))).merge(dynamic.createFloat(iterator.next().asFloat(0.0F))).merge(dynamic.createFloat(iterator.next().asFloat(0.0F))).merge(dynamic.createFloat(iterator.next().asFloat(0.0F)));
