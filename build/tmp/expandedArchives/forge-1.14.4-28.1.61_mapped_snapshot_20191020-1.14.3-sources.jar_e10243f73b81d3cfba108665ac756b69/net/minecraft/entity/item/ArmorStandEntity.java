...
public class ArmorStandEntity extends LivingEntity {
      ...
      if (!this.hasMarker() && itemstack.getItem() != Items.NAME_TAG) {
         if (!this.world.isRemote && !player.isSpectator()) {
            ...
            if (itemstack.isEmpty()) {
               ...
               if (this.hasItemInSlot(equipmentslottype2)) {
                  this.swapItem(player, equipmentslottype2, itemstack, hand);
               }
            } else {
               ...
               this.swapItem(player, equipmentslottype, itemstack, hand);
            }
   ...
   protected EquipmentSlotType getClickedSlot(Vec3d p_190772_1_) {
   ...
   public boolean isDisabled(EquipmentSlotType slotIn) {
      return (this.disabledSlots & 1 << slotIn.getSlotIndex()) != 0 || slotIn.getSlotType() == EquipmentSlotType.Group.HAND && !this.getShowArms();
   }

   private void swapItem(PlayerEntity player, EquipmentSlotType p_184795_2_, ItemStack p_184795_3_, Hand hand) {
   ...
   private void playParticles() {
      if (this.world instanceof ServerWorld) {
         ((ServerWorld)this.world).spawnParticle(new BlockParticleData(ParticleTypes.BLOCK, Blocks.OAK_PLANKS.getDefaultState()), this.posX, this.posY + (double)this.getHeight() / 1.5D, this.posZ, 10, (double)(this.getWidth() / 4.0F), (double)(this.getHeight() / 4.0F), (double)(this.getWidth() / 4.0F), 0.05D);
      }
   ...
   private void playBrokenSound() {
      this.world.playSound((PlayerEntity)null, this.posX, this.posY, this.posZ, SoundEvents.ENTITY_ARMOR_STAND_BREAK, this.getSoundCategory(), 1.0F, 1.0F);
   }
